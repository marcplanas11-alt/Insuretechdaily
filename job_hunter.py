...
Domain skills: {', '.join(PROFILE['experience'][0]['skills'][:10])}
Technical skills: {', '.join(PROFILE['technical_skills'])}
Languages: {', '.join(PROFILE['languages'])}
Minimum salary: €{PROFILE['min_salary_eur']:,}/year
Location preference: Remote EU ✅ | Hybrid Barcelona ✅ | Hybrid elsewhere ❌ | On-site outside Barcelona ❌
...
HARD FILTERS (score 0 immediately if any apply):
- Salary explicitly below €60,000/year
- Role is on-site only outside Barcelona
- Role is hybrid but NOT in Barcelona or Spain
- Role requires relocation outside EU
- Role is junior, graduate, intern, or entry-level

LOCATION RULES:
- ACCEPT: Fully remote (EU or global) ✅
- ACCEPT: Hybrid in Barcelona or Spain ✅
- REJECT: Hybrid anywhere else in Europe ❌
- REJECT: On-site only (outside Barcelona) ❌

TECHNICAL SKILLS TO HIGHLIGHT (candidate has all of these):
Python, SQL, Agile/Scrum, Excel/Data Analysis, CRM Systems
...
TASKS:
1. Apply hard filters first — if any fail, return score: 0
2. Score 0-100 based on: role relevance, domain match, seniority, location fit, salary fit
3. If score >= 80, write:
   a) Tailored CV summary in English (3-4 sentences, include relevant technical skills)
   b) Tailored CV summary in Spanish (3-4 sentences, include relevant technical skills)
   c) Cover letter opening in English (3-4 sentences)
   d) Cover letter opening in Spanish (3-4 sentences)

Respond ONLY in valid JSON:
{{
  "score": <integer 0-100>,
  "reason": "<one sentence why, mention location/salary if filtered>",
  "location_type": "<remote|hybrid-barcelona|hybrid-other|onsite>",
  "salary_info": "<salary found in listing or 'not specified'>",
